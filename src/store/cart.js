...
//UseProductStore
import { userProductStore } from './product';
...
export const useStore = defineStore('useStore',()=>{
    ...
    const cart  =ref([])
      ...
      const add_cart = (id, price, quantity = 1) => {
        ...
        const finId = cart.value.find(e=>e.id === data.id)
    ...
    const cart_previews = computed(()=>{
       ...
       return cart.value.map((prd,i)=>{
           ...
           return {
                ...
                total_product :  cart.value[i].quantity  * parseFloat(cart.value[i].price)
            }
    ...
    return {add_cart,loadFromLocalStorrage,cart_previews,increment_quantity,decrement_quantity,remove_cart,clear_cart,add_cart_detail,total}
})